...
> 오디오 없이 되는 6건을 먼저 끝냈고, C1은 무료 Web Speech 범위에서 완료했다. C8·WS10은 보류한다.
...
| 완료 | C1 | 듣기 트랙(받아쓰기·청해) | 이 문서 아래 절(완료 기록) | 중 | 아니오 |
...
✅ done 2026-07-08 — Web Speech 기반 C1 듣기 트랙을 먼저 완성했다. `src/lib/listening.js`가 각 챕터의 `grammarNotes.examples`와 dialogue lines에서 짧은 문장을 런타임 파생하고, LessonPlayer practice 단계에 받아쓰기 2개 + 듣고 고르기 2개를 추가한다. Practice 탭에는 브라우저가 Korean Web Speech를 지원할 때만 Listening 모드가 뜨며, 선택한 챕터 또는 전체 챕터에서 8문항 세트를 만든다. `ListeningSession`은 텍스트 숨김, 3회 재생 제한, 0.8x 속도 토글, 받아쓰기 부분 판정, 결과 후 Korean/romanization/English reveal을 제공한다. 검증: C1 단위 테스트, 전체 테스트, build, content lint, Playwright 수동 QA를 통과했다. C8 발음 미니멀 페어와 WS10 고품질 오디오는 여전히 별도 승인 전 보류다.
