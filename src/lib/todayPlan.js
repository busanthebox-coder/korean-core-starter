...
export function buildTodayPlan({ dueCount = 0, currentChapter = null, spoken = {}, today = dayKey() } = {}) {
  const steps = [];
  if (dueCount > 0) {
    steps.push({ kind: 'review', label: `Review ${dueCount} due`, target: '/practice?review=1&today=1' });
  }
  if (currentChapter) {
    steps.push({ kind: 'lesson', label: `Continue Chapter ${currentChapter.number}`, target: currentChapter.id });
