...
describe('WordsScreen', () => {
  it('keeps the core word visible and opens the example only on request', async () => {
    render(WordsScreen, {
      items: [{
        ko: '약속',
        romanization: 'yaksok',
    ...
    const disclosure = screen.getByText('Example & why').closest('details');
    expect(disclosure).not.toHaveAttribute('open');

    await fireEvent.click(screen.getByText('Example & why'));
