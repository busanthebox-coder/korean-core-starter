const DATA_FILES = [
  './data/words.json',
  './data/newcomer-vocab.json',
  './data/vocab-extended.json',
...
const DATA_BUNDLE_KEYS = {
  ...
  './data/newcomer-vocab.json': 'newcomerVocab',
  './data/vocab-extended.json': 'extendedVocab',
...
async function init() {
  try {
    const [payloads, course, grammar, activities, guide, dialogues] = await Promise.all([
      ...
      fetchJson(TEXTBOOK_FILES.guide),
      fetchJson(TEXTBOOK_FILES.dialogues).catch(() => ({ dialogues: [] }))
    ]);
    ...
    state.guideTracks = guide.tracks || [];
    state.guideTrackId = state.guideTracks[0]?.id || null;
    state.dialogues = dialogues.dialogues || [];
    state.dialogueId = state.dialogues[0]?.id || null;
    wireGlobalEvents();
    renderFilters();
    renderCourse();
    renderBrowse();
    setRailVisible(state.mode);
  } catch (err) {
...
function setRailVisible(mode) {
  // The left filter rail only applies to Browse; hide it elsewhere so other screens go full-width.
  const shell = document.querySelector('.app-shell');
  if (shell) shell.classList.toggle('rail-hidden', mode !== 'browse');
}
...
function renderGuide() {
  ...
  const tracks = state.guideTracks || [];
  if (!tracks.length) {
    view.innerHTML = '<div class="empty-state"><div><h1>Guide unavailable.</h1><p>Could not load the newcomer guide.</p></div></div>';
    return;
  }
  const track = tracks.find(t => t.id === state.guideTrackId) || tracks[0];
  state.guideTrackId = track.id;
  const units = track.units || [];
  const unit = units.find(u => u.id === state.guideUnitId) || units[0];

  view.innerHTML = `
    <section class="course-hero">
      <div class="eyebrow">Just arrived in Korea</div>
      <h1>Newcomer Guide</h1>
      <p>Survival Korean for your first weeks in Korea, organized by real situations. Each unit teaches the phrases first — with a dialogue, linked vocabulary, and drills — then adds the practical steps and official links you need.</p>
    </section>
    <nav class="course-jump guide-tracks" aria-label="Guide tracks">
      ${tracks.map(t => `<button type="button" class="${t.id === track.id ? 'is-active' : ''}" data-guide-track="${esc(t.id)}">${esc(t.letter)} · ${esc(t.title)}</button>`).join('')}
    </nav>
    <div class="textbook-note"><strong>${esc(track.title)}</strong><span>${esc(track.summary)}</span></div>
    <div class="guide-unit-list">
      ${units.map(u => `<button type="button" class="compact-entry-link ${unit && u.id === unit.id ? 'is-active' : ''}" data-guide-unit="${esc(u.id)}">
        <span class="badge type-pattern">Unit</span>
        <strong>${esc(u.title)}</strong>
        <span>${esc(u.situation)}</span>
      </button>`).join('')}
    </div>
    ${unit ? renderGuideUnit(unit) : ''}
  `;
  wireGuideActions();
}

function renderGuideUnit(unit) {
  const vocab = (unit.coreVocabularyIds || []).map(findEntry).filter(Boolean);
  const linked = (unit.linkedEntryIds || []).map(findEntry).filter(Boolean);
  const deepLinks = unit.deepLinks || [];
  return `
    <article class="unit-card guide-unit" id="${esc(unit.id)}">
      <div class="unit-number">Survival unit</div>
      <div class="unit-main">
        <h2>${esc(unit.title)}</h2>
        <p class="unit-outcome">${esc(unit.goal)}</p>
        <div class="unit-meta"><span>${esc(unit.situation)}</span><span>${(unit.keyPhrases || []).length} key phrases</span></div>

        ${renderChapterBeginnerGuide(unit)}

        <div class="unit-section-label">Key phrases</div>
        ${renderUsagePhrases({ usagePhrases: unit.keyPhrases || [] })}

        ${renderWarmupDialogue(unit.dialogue || [])}

        ${unit.drills && unit.drills.length ? `<div class="unit-section-label">Drills</div>${renderUsagePhrases({ usagePhrases: unit.drills })}` : ''}

        ${vocab.length ? `<div class="unit-section-label">Vocabulary</div><div class="unit-link-grid">${vocab.map(renderCompactEntryLink).join('')}</div>` : ''}
        ${linked.length ? `<div class="unit-section-label">Patterns &amp; expressions</div><div class="unit-link-grid">${linked.map(renderCompactEntryLink).join('')}</div>` : ''}

        ${unit.checkpoints && unit.checkpoints.length ? `<div class="chapter-checkpoints"><strong>Before you go</strong><ul>${unit.checkpoints.map(c => `<li>${esc(c)}</li>`).join('')}</ul></div>` : ''}

        ${unit.steps && unit.steps.length ? `<div class="unit-section-label">Steps</div><ol class="guide-steps">${unit.steps.map(s => `<li>${esc(s)}</li>`).join('')}</ol>` : ''}
        ${unit.costs ? `<div class="unit-task"><strong>Cost</strong><span>${esc(unit.costs)}</span></div>` : ''}
        ${unit.notes ? `<div class="friend-practice"><strong>Good to know</strong><span>${esc(unit.notes)}</span></div>` : ''}
        ${deepLinks.length ? `<div class="unit-section-label">Official links</div><div class="guide-deeplinks">${deepLinks.map(l => `<a class="action-button" href="${esc(l.url)}" target="_blank" rel="noopener noreferrer">${esc(l.label)}${l.note ? ` — <span class="muted">${esc(l.note)}</span>` : ''}</a>`).join('')}</div>` : ''}
      </div>
    </article>
  `;
}
...
function wireGuideActions() {
  document.querySelectorAll('#guideView [data-guide-track]').forEach(btn => {
    btn.addEventListener('click', () => {
      state.guideTrackId = btn.dataset.guideTrack;
      state.guideUnitId = null;
      renderGuide();
    });
  });
  document.querySelectorAll('#guideView [data-guide-unit]').forEach(btn => {
    btn.addEventListener('click', () => {
      state.guideUnitId = btn.dataset.guideUnit;
      renderGuide();
      document.getElementById(btn.dataset.guideUnit)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  });
  document.querySelectorAll('#guideView [data-entry]').forEach(card => {
    card.addEventListener('click', () => openDetail(card.dataset.entry));
  });
  document.querySelectorAll('#guideView [data-speak]').forEach(btn => {
...
function renderTalk() {
  ...
  if (!dialogues.length) {
    view.innerHTML = '<div class="empty-state"><div><h1>No dialogues yet.</h1><p>Could not load dialogues.</p></div></div>';
    return;
  }
  const current = dialogues.find(d => d.id === state.dialogueId) || dialogues[0];
  state.dialogueId = current.id;
  view.innerHTML = `
    <section class="course-hero">
      <div class="eyebrow">Speak like a local</div>
      <h1>Talk — Situational Dialogues</h1>
      <p>Real beginner conversations by situation. Tap any line to hear it, then use Practice mode to hide the English and rehearse your part.</p>
    </section>
    <div class="talk-list">
      ${dialogues.map(d => `<button type="button" class="talk-card ${d.id === current.id ? 'is-active' : ''}" data-talk="${esc(d.id)}">
        <span class="badge type-expression">${esc(d.level || 'A1')}</span>
        <strong>${esc(d.title)}</strong>
        <span>${esc(d.situation)}</span>
      </button>`).join('')}
    </div>
    ${renderDialogueDetail(current)}
  `;
  wireTalkActions();
}

function renderDialogueDetail(d) {
  const reveal = state.dialogueReveal;
  return `
    <article class="unit-card dialogue-card" id="${esc(d.id)}">
      <div class="unit-main">
        <h2>${esc(d.title)}</h2>
        <p class="unit-outcome">${esc(d.situation)}</p>
        <div class="dialogue-toolbar">
          <button type="button" class="small-button" data-talk-toggle>${reveal ? 'Practice mode · hide English' : 'Show English'}</button>
        </div>
        <div class="talk-dialogue ${reveal ? '' : 'is-hidden-en'}">
          ${(d.lines || []).map(line => `
            <div class="talk-line">
              <div class="dialogue-speaker">${esc(line.speaker)}</div>
              <div class="talk-line-body">
                ...
                ${line.note ? `<div class="usage-note">${esc(line.note)}</div>` : ''}
              </div>
            </div>`).join('')}
        </div>
        ${d.tip ? `<div class="friend-practice"><strong>Tip</strong><span>${esc(d.tip)}</span></div>` : ''}
      </div>
    </article>
  `;
}

function wireTalkActions() {
  document.querySelectorAll('#talkView [data-talk]').forEach(btn => {
    btn.addEventListener('click', () => {
      state.dialogueId = btn.dataset.talk;
      renderTalk();
      document.getElementById(btn.dataset.talk)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  });
  const toggle = document.querySelector('#talkView [data-talk-toggle]');
  if (toggle) toggle.addEventListener('click', () => { state.dialogueReveal = !state.dialogueReveal; renderTalk(); });
  document.querySelectorAll('#talkView [data-speak]').forEach(btn => {
...
function renderCourse() {
  ...
  $('courseView').innerHTML = `
    <section class="learn-hero" id="course-top">
      <div class="eyebrow">Beginner Korean · A1–A2</div>
      <h1>Your lessons</h1>
      <p>${chapters.length} chapters · ${state.entries.length} cards. Tap a chapter to start, then work down.</p>
    </section>
    <section class="section" id="course-chapters">
      <div class="unit-list lesson-path">
        ${chapters.map((chapter, index) => renderChapter(chapter, index, chapters)).join('')}
      </div>
    </section>
    <section class="section extras-section">
      <div class="extras-head">Reference</div>
      <details class="course-collapse" id="course-grammar">
        <summary><span>Grammar reference</span><span class="muted">${state.grammarItems.length + state.endingItems.length} points · particles, endings, tenses, connectors</span></summary>
        ${renderParticleGuide()}
      </details>
      <details class="course-collapse" id="course-map">
        <summary><span>Study map</span><span class="muted">how this beginner course is organized</span></summary>
  ...
  return `
    <section class="course-jump-wrap" aria-label="Course quick navigation">
      <div class="course-jump">
        <button type="button" data-course-jump="course-grammar">Grammar</button>
...
const GRAMMAR_CATEGORIES = [
  { label: 'Foundations', blurb: 'Reading and sound basics.', test: id => id.startsWith('grammar-foundation-') },
  { label: 'Particles', blurb: 'Small endings that show each word’s job: topic, subject, object, place, method.', test: id => id.startsWith('grammar-particle-') },
  { label: 'Sentence endings', blurb: 'Wants, ability, requests, suggestions, and speaker attitude.', test: id => id.startsWith('grammar-ending-') },
  { label: 'Tense & aspect', blurb: 'Past, future, and actions in progress.', test: id => id.startsWith('grammar-tense-') },
  { label: 'Connectors', blurb: 'Join clauses: reason, sequence, condition, contrast.', test: id => id.startsWith('grammar-connector-') },
  { label: 'Time relations', blurb: 'Before and after doing something.', test: id => id.startsWith('grammar-time-') },
  { label: 'Quantity & comparison', blurb: 'Comparing, pluralizing, and counting with counters.', test: id => id.startsWith('grammar-compare-') || id.startsWith('grammar-plural-') || id === 'grammar-counter' },
  { label: 'Word forms & exceptions', blurb: 'Noun-describing forms (뜨거운 vs 뜨거울) and irregular stems.', test: id => id.startsWith('grammar-modifier-') || id.startsWith('grammar-irregular-') },
  { label: 'Politeness', blurb: 'Honorific forms that respect the person you talk about.', test: id => id.startsWith('grammar-honorific-') }
];

function renderParticleGuide() {
  const all = [...(state.grammarItems || []), ...(state.endingItems || [])];
  if (!all.length) {
    return `<div class="particle-grid">${PARTICLE_GUIDES.map(renderParticleCard).join('')}</div>`;
  }
  const used = new Set();
  const sections = GRAMMAR_CATEGORIES.map(cat => {
    const items = all.filter(it => !used.has(it.id) && cat.test(it.id));
    items.forEach(it => used.add(it.id));
    if (!items.length) return '';
    return `
      <details class="grammar-cat">
        <summary>
          <span class="grammar-cat-title">${esc(cat.label)} <span class="count-chip">${items.length}</span></span>
          <span class="grammar-cat-blurb">${esc(cat.blurb)}</span>
        </summary>
        <div class="particle-grid">${items.map(renderParticleCard).join('')}</div>
      </details>
    `;
  }).join('');
  const leftover = all.filter(it => !used.has(it.id));
  const leftoverHtml = leftover.length
    ? `<div class="particle-intro form-section-head"><strong>More</strong></div><div class="particle-grid">${leftover.map(renderParticleCard).join('')}</div>`
    : '';
  return `
    <div class="particle-intro">
      <strong>Grammar reference — ${all.length} points</strong>
      <span>Beginner Korean grammar in one place: particles, sentence endings, tenses, connectors, time, quantity, word forms, and politeness. Each card gives a rule, examples, common mistakes, and quick practice.</span>
    </div>
    <div class="particle-compare">
      <div><strong>은/는 vs 이/가</strong><span>은/는 sets the topic or contrast. 이/가 points to the subject or new information.</span><em>저는 학생이에요. 시간이 있어요.</em></div>
      <div><strong>에 vs 에서</strong><span>에 marks a destination, time, or where something exists. 에서 marks where an action happens.</span><em>학교에 가요. 학교에서 공부해요.</em></div>
      <div><strong>기 전에 vs 은 후에</strong><span>기 전에 = before doing; 은/ㄴ 후에 = after doing.</span><em>먹기 전에 / 먹은 후에</em></div>
    </div>
    ${sections}
    ${leftoverHtml}
  `;
...
function renderChapter(chapter, index, chapters = []) {
  ...
  return `
    <details class="unit-card chapter-card" id="${esc(chapter.id)}">
...
function scrollToCourseTarget(id) {
  ...
  if (!target) return;
  // If the target (or an ancestor) is a collapsed <details>, open it so the content is visible.
  ...
  return `
    <article class="detail">
      ...
      <div class="learning-sections">
        ...
        ${e.conjugationTips && e.conjugationTips.length ? renderLearningSection('Conjugation tips', renderConjugationTips(e), true) : ''}
        ${renderLearningSection('Examples', renderExamples(e), true)}
        ${e.lesson ? renderLearningSection('Lesson', renderLesson(e), false) : ''}
        ${(e.forms || e.patternInfo || e.adjectiveForms || e.speechLevels) ? renderLearningSection('Forms', renderFormsSection(e), false) : ''}
        ${(e.studyGuide || e.relatedPatternIds?.length || e.relatedWordIds?.length) ? renderLearningSection('Connections', renderConnections(e), false) : ''}
...
function renderConjugationTips(e) {
  return `
    <div class="conj-tips">
      ${(e.conjugationTips || []).map(t => `
        <div class="conj-tip">
...
function renderLesson(e) {
  ...
  if (!lesson) return '';
  const dialogue = lesson.miniDialogue || [];
